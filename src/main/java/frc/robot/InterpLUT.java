...
// From: https://github.com/FTCLib/FTCLib/blob/master/core/src/main/java/com/arcrobotics/ftclib/util/InterpLUT.java
/**
...
public class InterpLUT {
    ...
    public double get(double input) {
        ...
        if (input <= mX.get(0)) {
            ...
            var base = mY.get(0);
            var slope = mM.get(0);
            return base + (slope * (input - x));
            // throw new IllegalArgumentException("User requested value outside of bounds of
            // LUT. Bounds are: "
            // + mX.get(0).toString() + " to " + mX.get(n - 1).toString() + ". Value
            // provided was: " + input);
        }
        // Extrapolate oob
        if (input >= mX.get(n - 1)) {
